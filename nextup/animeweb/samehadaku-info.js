...
module.exports = {
    ...
    start: async(client, m, { text }) => {
        ...
        client.sendFile(m.from, search?.thumb, "", m, { caption })
        let url = search?.episode_list
        for (let a of url) {
            if (!a) return m.reply("Episode Not Found!")
            array.push([a.title, [[a.title, `.samehadaku-download ${a.link}`]]])
        }
        client.sendList(m.from, "Samehadaku Detail", "This is a list to download one of the episodes", config.footer, "Choose", array, m, { previewType: 4 })
    },
    isQuery: true
}