...
module.exports = {
    ...
    start: async(client, m, { text }) => {
        if (isUrl(text)) {
            ...
            client.sendFile(m.from, fetch.result[0], "", m, { caption: `Download Story From : ${isUrl(text)[0]}` })
        } else {
            let fetch = await fetchUrl(global.api("zenz", "/downloader/instagram/story/username", { query: text }, "apikey"))
            for (let i of fetch.result.url) client.sendFile(m.from, i, "", m, { caption: `Download Story From : ${text}` })
        }
    },
    isQuery: true
}
