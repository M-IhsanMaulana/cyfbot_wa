...
module.exports = {
    ...
    start: async (client, m, { text }) => {
        if (m.text.toLowerCase().endsWith("audio")) {
            client.sendMessage(m.from, { audio: { url: isUrl(text)[0] }, mimetype: "audio/mpeg" }, { quoted: m })
        } else if (m.text.toLowerCase().endsWith("video")) {
            client.sendMessage(m.from, { video: { url: text.split(" ")[0] }, mimetype: "video/mp4" }, { quoted: m })
        } else {
