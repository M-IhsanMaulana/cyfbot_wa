...
`
            ...
            client.sendText(m.from, teks, m)
            if (c && c.toLowerCase() == "--audio") client.sendFile(m.from, global.api("zenz", `/islami/quran/audio/${b}`, {}, "apikey"), name.long, m)
...
⭔ Tafsir : ${tafsir.id.long}           
            `
            client.sendText(m.from, teks, m)
            if (d && d.toLowerCase() == "--audio") client.sendFile(m.from, global.api("zenz", `/islami/quran/audio/${b}/${c}`, {}, "apikey"), surah.name.long, m)
        } else {
            return m.reply(`
Example :
