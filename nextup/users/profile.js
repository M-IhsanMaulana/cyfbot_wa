...
module.exports = {
	...
	noLimit: true,
    start: async(client, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await client.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await client.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		client.sendFile(m.from, pp, "", m, { caption })
    }
}