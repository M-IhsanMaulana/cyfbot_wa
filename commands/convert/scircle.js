...
module.exports = {
    ...
    start: async(client, m, { command, prefix, quoted, mime, text }) => {
        ...
        if (/image/.test(mime)) {
            ...
            client.sendFile(m.from, global.api("zenz", "/photoeditor/circle", { url: isUrl(text)[0] }, "apikey"), "", m, { asSticker: true, author: config.exif.author, packname: config.exif.packname, categories: ['😄','😊'] })
        }   else {
