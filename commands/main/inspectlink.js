...
module.exports = {
    ...
    start: async(client, m, { text }) => {
        ...
        let res = await client.groupQueryInvite(code)
...
⬡ *Friends Who Are Known to Join :*\n${res.participants ? res.participants.map((user, i) => ++i + ". @" + user.jid.split("@")[0]).join("\n").trim() : "Not Found"}
        ...
        let pp
        try {
            pp = await client.profilePictureUrl(res.id, "image")
        } catch {
            pp = "https://tse2.mm.bing.net/th?id=OIP.n1C1oxOvYLLyDIavrBFoNQHaHa&pid=Api&P=0&w=153&h=153"
        }
        client.sendFile(m.from, pp, "", m, { caption: teks, mentions: await client.parseMention(teks) })
    },
    isQuery: true
}
