...
module.exports = async (client, m, commands, chatUpdate) => {
    try {
        ...
        let isOwner = [client.user?.jid, ...config.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)
        ...
        premium.expiredCheck(client, m, _premium);
        ...
        let cyfbot = (m.quoted || m)
        let cyf = (cyfbot.mtype == 'buttonsMessage') ? cyfbot[Object.keys(cyfbot)[1]] : (cyfbot.mtype == 'templateMessage') ? cyfbot.hydratedTemplate[Object.keys(cyfbot.hydratedTemplate)[1]] : (cyfbot.mtype == 'product') ? cyfbot[Object.keys(cyfbot)[0]] : (cyfbot.mtype == 'viewOnceMessage') ? cyfbot.message[Object.keys(cyfbot.message)[0]] : m.quoted ? m.quoted : m
        let quoted = (cyf.msg || cyf)
        let mime = (cyf.msg || cyf).mimetype || ''
        ...
        if (isCmd && !isGroup) {
            console.log(
              color(`[| ${timeNow} |]=%> [ SERVER ] ~> `, "cyan"),
              color(`Type : [ COMMAND ] ~> `, "#A1FFCE"),
              color(`[ DATA ] ~>`, "#38ef7d"),
              color(`${cmdName} Type Message : ${type}`, "blue"),
              color("~> From ", "yellow"),
              color(pushname, "#fff")
            );
            ...
            console.log(
              ...
              color(`${cmdName} Type Message : ${type}`, "blue"),
              color("~> From ", "yellow"),
              color(`${m.pushName} in`, "#38ef7d"),
              color(isGroup ? metadata.subject : m.pushName, "#fff")
            )
        }
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            messages.key.fromMe = await areJidsSameUser(m.sender, client.user.jid)
        ...
        //     let msg = await client.serializeM(await Store.loadMessage(key.remoteJid, key.id))
        ...
        //     client.relayMessage(m.from, msg.message, { messageId: msg.id })
        //     await client.sendText(m.from, tekss, msg, { mentions: [msg.sender] })
        // }
